...
describe('UI Store', () => {
  ...
  describe('초기 상태', () => {
    ...
    it('사용자 설정이 기본값으로 설정되어야 한다', () => {
      const { result } = renderHook(() => usePreferences())
      expect(result.current.preferences.reducedMotion).toBe(false)
      expect(result.current.preferences.autoplaySlideshow).toBe(true)
      expect(result.current.preferences.showArtworkDetails).toBe(true)
  ...
  describe('사용자 설정 관리', () => {
    it('개별 설정을 변경할 수 있어야 한다', () => {
      const { result } = renderHook(() => usePreferences())
      
      act(() => {
        result.current.setPreference('reducedMotion', true)
      })
      
      expect(result.current.preferences.reducedMotion).toBe(true)
      
      act(() => {
        result.current.setPreference('gridSize', 'large')
      })
      
      expect(result.current.preferences.gridSize).toBe('large')
    })

    it('boolean 설정을 토글할 수 있어야 한다', () => {
      const { result } = renderHook(() => usePreferences())
      
      expect(result.current.preferences.autoplaySlideshow).toBe(true)
      
      act(() => {
        result.current.togglePreference('autoplaySlideshow')
      })
      
      expect(result.current.preferences.autoplaySlideshow).toBe(false)
